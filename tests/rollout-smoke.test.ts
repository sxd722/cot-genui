...
describe("adaptive rollout smoke matrix", () => {
  ...
  it("allows every adaptive/edit/reflection flag to fail closed independently", () => {
    const env = Object.fromEntries(["ADAPTIVE_QUERY_CLASSIFICATION", "ADAPTIVE_STEERING", "PROFILE_VIEW_V2", "WEB_FACTS_OPTIONAL", "OPENUI_CARD_EDIT", "REFLECTION_ATTRIBUTION", "REFLECTION_GRADIENT", "GUARDED_AUTO_LEARN"].map((name) => [`NEXT_PUBLIC_${name}`, "false"]));
