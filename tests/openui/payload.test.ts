...
function legacyPayload(cardPlan: typeof sampleCardPlan) {
  ...
  return {
    ...
    actionBindings: cardPlan.cards.flatMap((card) => (card.actions ?? []).map((action) => ({
      ref: `plan:${encodeURIComponent(card.id)}:${encodeURIComponent(action.id)}`,
      cardId: card.id,
      actionId: action.id,
      label: action.label,
      type: action.type,
...
describe("OpenUI model payload", () => {
  it.each([
    ...
    ["6 cards", sixCardPlan],
  ])("sends only CardPlan Markdown and required shell for %s", (_label, cardPlan) => {
    const payload = buildOpenUIGenerationPayload(cardPlan);

    expect(Object.keys(payload)).toEqual(["cardPlanMarkdown", "requiredShell"]);
  ...
  it("does not resend Markdown or action bindings during repair", () => {
