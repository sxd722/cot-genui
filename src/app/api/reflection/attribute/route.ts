...
export async function POST(request: Request) {
  ...
  try {
    ...
    const value = await callReflectionJson(ATTRIBUTION_SYSTEM_PROMPT, { episode: view, inferredEditIntents: intents, attributionPrior: attributionPrior(intents), instruction: "Only move probability away from the prior when topTargets includes concrete field or short-text evidence." });
