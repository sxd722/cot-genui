...
export async function callReflectionJson(system: string, user: unknown, profile: ModelProfile): Promise<unknown> {
  ...
  const params = {
    ...
    ...(target.provider === "glm" ? { thinking: { type: target.thinking ? "enabled" : "disabled" }, do_sample: true } : {}),
