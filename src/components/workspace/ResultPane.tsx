...
export function ResultPane() {
  ...
  const canShowOpenUI = !!cardPlan && (streaming || !!generatedSource || (isStitch && steps.openui_generate.status === "error"));
  ...
  const active = (requested === "cardplan-markdown" && !cardPlanMarkdown)
    ...
    || (requested === "openui-source" && !generatedSource) ? fallback : requested;
  return (
    <section className="flex min-h-0 flex-col overflow-hidden border-b border-zinc-200 bg-zinc-950 dark:border-zinc-800">
      <div className="flex shrink-0 items-center gap-2 border-b border-zinc-800 px-3 py-2">
        ...
        <select value={active ?? ""} disabled={!active} onChange={(event) => setRightView(event.target.value as ResultView)} className="rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-[11px] text-zinc-300">
          ...
          <option value="openui" disabled={!canShowOpenUI}>✨ {isStitch ? "Stitch H5 渲染" : "OpenUI 渲染"}</option>
          <option value="openui-source" disabled={!generatedSource}>⌨️ {isStitch ? "Stitch H5 源码" : "OpenUI 源码"}</option>
        </select>
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        ...
        {active === "openui-source" && generatedSource ? <pre className="h-full overflow-auto p-3 font-mono text-[10px] leading-relaxed text-cyan-300/80">{generatedSource}</pre> : null}
