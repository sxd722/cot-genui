  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-6" role="dialog" aria-modal="true" aria-label="反思学习">
    <div className="flex max-h-[88vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
        <div className="grid gap-1 text-[11px]"><p className="text-emerald-600">✓ 已保存最终版本</p><p className={report ? "text-emerald-600" : "text-zinc-400"}>{report ? "✓" : "◌"} 已汇总局部编辑与整体反馈</p><p className={report ? "text-emerald-600" : "text-zinc-400"}>{report ? "✓" : "◌"} 已进行推理阶段归因</p><p className={state.reflectionStatus === "ready" ? "text-emerald-600" : "text-zinc-400"}>{state.reflectionStatus === "ready" ? "✓" : "◌"} {state.reflectionStatus === "generating-candidates" ? "正在生成可学习的 steering 候选" : "策略候选处理"}</p></div>
        {state.currentEpisode?.feedback?.length ? <section className="rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs text-violet-900 dark:border-violet-900 dark:bg-violet-950/30 dark:text-violet-200"><h3 className="font-semibold">本次整体卡片流反馈</h3><ul className="mt-1 list-disc space-y-1 pl-4">{state.currentEpisode.feedback.map((feedback) => <li key={feedback.id}>{feedback.text}</li>)}</ul><p className="mt-2 text-[10px] text-violet-600 dark:text-violet-400">这些内容只参与本次反思，不会回写当前卡片。</p></section> : null}
